...
getData() {
    ...
    if [[ "$(needNginx)" = "no" ]]; then
        if [[ "$TLS" = "true" ]]; then
            read -p " 请输入xray监听端口[强烈建议443，默认3443]：" PORT
            [[ -z "${PORT}" ]] && PORT=3443
        else
...
module_hotfixes=true' > /etc/yum.repos.d/nginx.repo
        ...
        $CMD_INSTALL nginx
        if [[ "$?" != "0" ]]; then
            colorEcho $RED " Nginx安装失败，请到 https://hijk111.art 反馈"
            exit 1
        fi
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        curl -sL https://get.acme.sh | sh -s email=hi111jk.pw@protonmail.sh
        source ~/.bashrc
...
server {
    ...
    listen [::]:80;
    server_name ${DOMAIN};
    return 301 https://\$server_name:3442\$request_uri;
}

server {
    listen       3442 ssl http2;
    listen       [::]:3442 ssl http2;
    server_name ${DOMAIN};
    charset utf-8;
...
installXray() {
    rm -rf /tmp/xray
    mkdir -p /tmp/xray
    DOWNLOAD_LINK="${V6_PROXY}https://github.com/XTLS/Xray-core/releases/download/v24.12.31/Xray-linux-$(archAffix).zip"
...
Documentation=https://github.com/xtls https://hi111jk.art
...
install() {
    ...
    if [[ $RETVAL == 0 ]]; then
        ...
        colorEcho $BLUE " 安装Xray v24.12.31 ，架构$(archAffix)"
        installXray
    fi
